...
#plt.rcParams['animation.ffmpeg_path'] = '/usr/local/Cellar/ffmpeg/4.3_2/bin/ffmpeg' #my add - this path needs to be added
...
def animateMe(Ts, t_all, states_all, cmds_all, targets_all, obstacles_all, walls_plots, showObs, centroid_all, f, tactic_type, pins_all):
    ...
    line_ani = animation.FuncAnimation(fig=fig, func=update, blit=False, frames=len(t_all[0:-2:numFrames]), interval=(Ts*1000*numFrames))
    line_ani.save('Figs/animation.gif', writer=writer)
    #line_ani.save('Figs/animation.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
    plt.show()
    return line_ani
